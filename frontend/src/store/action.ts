...
import LoggedUserRdo from '../dto/user/logged-user.rdo';
...
export const postOffer = createAsyncThunk<Offer, NewOffer, { extra: Extra }>(
  ...
  async (newOffer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<OfferPreviewRdo>(ApiRoute.Offers, adaptNewOfferToServer(newOffer));
...
export const editOffer = createAsyncThunk<Offer, Offer, { extra: Extra }>(
  ...
  async (offer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.patch<OfferPreviewRdo>(`${ApiRoute.Offers}/${offer.id}`, adaptOfferToServer(offer));
    history.push(`${AppRoute.Property}/${data.id}`);
...
export const fetchPremiumOffers = createAsyncThunk<Offer[], string, { extra: Extra }>(
  ...
  async (cityName, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<OfferPreviewRdo[]>(`${ApiRoute.Premium}?city=${cityName}`);
